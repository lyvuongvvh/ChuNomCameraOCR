...
- [x] **Phase 5 — Mobile app.** Build the camera capture + result display app (iOS/Android or cross-platform), calling the backend API above.
  A mobile *web* app (PWA), not native/React-Native/Flutter — fastest to build and iterate on, one codebase for both iOS and Android, matching this project's hobby-stage scope. Camera capture uses `<input type="file" capture="environment">` (opens the phone's native camera app) rather than a live in-browser `getUserMedia` preview — confirmed this needs no HTTPS/secure-context (unlike `getUserMedia`) and isn't affected by a real, still-open WebKit bug that breaks `getUserMedia` specifically in installed-PWA mode on iOS. Served by the same FastAPI process as the backend by default (no separate server, no CORS), with optional split-hosting support (`web/config.js`'s `API_BASE_URL`, `api/config.py`'s `ALLOWED_ORIGINS`) for serving the frontend and backend from different origins (e.g. Cloudflare Pages + a tunneled backend). Verified against the real pipeline (not mocked): a real page image run through the actual browser UI end-to-end correctly detected and recognized readable text, and the translate-opt-in fail-fast path (503 with no API key configured) surfaced cleanly without losing already-rendered results.

  **Deployed to a real always-on host (Unraid), not just tested locally.** `docker-compose.yml` + a GitHub Actions workflow publish `api/`'s image to GHCR on every push to `main`. First real deploy attempt failed immediately - `tensorflow-cpu`'s official PyPI wheels require AVX and segfault (exit 139) on the Unraid host's CPU, a Xeon W3680 (Westmere, 2010), one generation before AVX existed (confirmed via `lscpu`, not assumed - no BIOS setting fixes a true silicon gap like this). Fixed with a community wheel built specifically `-march=westmere`, which surfaced a second, subtler failure: that wheel pulls in `jax`/`jaxlib` as one of its own dependencies, and jaxlib's official wheel has the identical AVX requirement - it crashes inside a `try/except ImportError` in TensorFlow's own code that only catches jax being *absent*, not jax being *present but incompatible* (a `RuntimeError`, not an `ImportError`). Removing jax/jaxlib (unused by this project) fixed it for good. Re-verified end to end after both fixes: the recognizer reproduced a known ground-truth transcription exactly (19/19 characters) and all 18 unit tests still passed. **Not yet done:** a real run on an actual phone, a real Anthropic-backed translate run, and wiring the Cloudflare Pages + Tunnel split-hosting setup end to end (backend is deployed and healthy on Unraid; frontend still needs to be published) — see [`web/README.md`](web/README.md).
