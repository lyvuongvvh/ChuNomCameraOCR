...
- [~] **Phase 3 — Translation integration.** `NomNaNMT` turned out not to exist as a usable tool (empty repo, listed as NomNaOCR's own TODO) — pivoted to a two-stage approach: a deterministic Hán-Nôm character reading dictionary (Stage 1), then an LLM fluency pass (Stage 2) to produce modern Vietnamese.
  **In progress.** Stage 1 built: combines Unicode's Unihan (`kVietnamese`) with a small community Nôm IME dictionary, covering only **51.3%** of this project's character vocabulary — a real, hard limitation of available data, not a bug. Stage 2 demonstrated manually on real ground-truth *and* real OCR-prediction lines, including at scale (11,215 baseline-error positions analyzed): post-correction (Phase 2b) recovers ~2.5x as many recognition errors as fine-tuning (Phase 2c) does uniquely, and the recovered characters are overwhelmingly meaning-changing (wrong dates, numbers, pronouns), not cosmetic — a stronger practical case for Phase 2b's output as the translation input than its accuracy numbers alone suggested. Not yet scripted as an automated pipeline — that needs an LLM API key decision, and there's no ground-truth modern-Vietnamese corpus to score full translations against automatically. See [`experiments/phase3_translation/results.md`](experiments/phase3_translation/results.md).
