# Project Context for Claude Code

This file is read automatically by Claude Code at the start of every session in this repo. It summarizes the project so you don't need to re-explain it each time — see `README.md` for full background.

## What this project is

Camera-based OCR + translation for Chữ Nôm (historical Vietnamese script). Core hypothesis: fine-tune a large-vocabulary historical **Chinese** OCR model (CHAT, via Kraken) on the **Nôm-specific** NomNaOCR dataset, producing a hybrid recognizer that's better than either alone.

**This hypothesis is unvalidated.** Do not skip straight to building the full pipeline — see "Working order" below.

## Key upstream references

- Nôm training data + original pipeline: https://github.com/ds4v/NomNaOCR
- Pretrained historical Chinese OCR model: https://github.com/colibrisson/CHAT_models
- Reference web app (Nôm-only, for comparison): https://github.com/ds4v/NomNaSite
- OCR engine used by CHAT (supports fine-tuning): https://kraken.re

## Working order — do not skip ahead

1. **Validate first.** Before writing any fine-tuning code, help me pull a handful of sample Nôm pages (from NomNaOCR's dataset or NomNaSite's demo) and run CHAT's pretrained model on them as-is. Count how many Chinese-derived (Chữ Hán) characters it correctly recognizes vs. NomNaOCR's own model on the same pages. This determines whether the whole hybrid approach is worth pursuing.
   **Done — result is negative.** CHAT scored 22.2% vs. NomNaOCR's own pretrained model at 86.7% (15 held-out pages, 679 Chữ Hán chars); a follow-up CHAT fine-tuning trial made it worse (4.1%). See `experiments/phase0_validation/results.md`.
2. ~~Only if step 1 shows a real improvement, proceed to building the Kraken fine-tuning pipeline on NomNaOCR's dataset.~~ **Not pursued** — Phase 0's result doesn't support it. NomNaOCR's own pretrained model is adopted as the baseline recognizer instead.
3. **Evaluate** (retargeted from evaluating a fine-tuned hybrid to evaluating NomNaOCR's own pretrained model, since step 2 was skipped) against NomNaOCR's original reported metrics (Sequence Accuracy, Character Accuracy, Character Error Rate), on its full held-out validation split.
   **Done.** Character Accuracy 84.7%, CER 0.14–0.15, but Sequence Accuracy (exact line match) only 29.4% — most lines have at least one character wrong despite high per-character accuracy. This is worth weighing before Phase 4/5: downstream translation will usually see near-correct, not exact, transcriptions. See `experiments/phase2_nomnaocr_baseline/results.md`.
   **Addendum (Phase 2b, post-correction) — done.** Before considering a full retraining pass, tried rescoring the recognizer's own beam-search output with a character n-gram LM, no weight changes. Result: modest real gain (Sequence Accuracy 29.4%→32.4% on held-out data never used for tuning), not a fix — a beam-search probe found some errors are confidently-wrong characters that never appear in any beam candidate, which no rescoring can recover. See `experiments/phase2b_postcorrection/results.md`.
   **Addendum (Phase 2c, fine-tuning) — done.** Actually continued training NomNaOCR's own model on `Train.txt` (8 epochs, Kaggle GPU) — unlike the CHAT trial, not cross-domain transfer, so no catastrophic forgetting. Result: modest gain, same order of magnitude as Phase 2b (Sequence Accuracy 29.4%→29.6-30.0%, Character Accuracy 84.7%→84.8-84.9%) that **plateaus almost immediately** — all 8 epoch checkpoints were evaluated, and essentially all the improvement happens by epoch 1, with epochs 2-8 flat within noise. More epochs of this same setup (fixed lr, no schedule/augmentation) would not be expected to help further; a genuinely different setup would be needed to test for a higher ceiling. Notable lesson: the in-training dev loss (necessarily carved from `Train.txt`, not `Validate.txt`) was actively misleading — the checkpoint it flagged as most overfit tied for *best* on the real held-out set. Always verify against the actual Phase 2 held-out pipeline, not a training loop's own loss signal. See `experiments/phase2c_finetune/results.md`.
4. **Only after a working, evaluated recognizer exists**, build the translation step (Nôm → modern Vietnamese) and the API/mobile app layers.
   **Done.** `NomNaNMT` (this file's own reference, above) turned out not to exist as a usable tool — empty placeholder repo, listed as NomNaOCR's own TODO. Pivoted to a two-stage approach: Stage 1 is a deterministic Hán-Nôm reading dictionary (Unihan + a small community Nôm IME dictionary + VNPF's "Bảng tra chữ Nôm" + Digitizing Vietnam's lookup, scraped live), covering 66.1% of this project's character vocabulary — a real limitation of available data, not a bug. Stage 2 is a scripted, parallelized LLM fluency pass (`translate_lib/llm_translate.py` + `scripts/translate.py`) against the real Anthropic API, defaulting to Phase 2b's post-corrected predictions (recovers ~2.5x as many recognition errors as Phase 2c's fine-tuning does uniquely, and those recoveries are overwhelmingly meaning-changing, not cosmetic).

   Ran the full 7,577-line corpus twice. First pass ($14.00, 0 errors) found poetry (Truyện Kiều, Lục Vân Tiên) self-flagging low confidence ~2x as often as prose — root-caused (not just observed) to the low-confidence trigger being calibrated for Classical Chinese grammar, a bar Nôm verse (characters chosen for sound, not sense) was never meant to meet. Fixed and validated on a 156-line sample ($0.33): low-confidence rate 66.0%→21.2%.

   Before re-running the corpus, built real ground truth for all three source works to measure actual translation accuracy, not just the confidence flag. Kiều/Lục Vân Tiên (already Vietnamese verse) aligned against complete Wikisource texts via an n-gram-seeded Levenshtein DP (`eval_lib/wikisource_alignment.py` — two cheaper cost functions were tried and gave wrong answers first, documented in results.md) — 1,823 + 407 lines aligned, each carrying its own confidence score. DVSKTT (genuine Literary Chinese prose, needing an actual translation rather than a spelling-normalized edition) aligned by physical page structure against the real 1993 published translation via its inline leaf markers (`eval_lib/dvsktt_ground_truth.py`) — 4,976 of 5,347 lines matched, a direct structural lookup rather than a fuzzy text match.

   Scoring against this ground truth (`eval_lib/scoring.py`) found the confidence flag itself is meaningful (self-flagged low-confidence lines score measurably lower against real ground truth than confident ones, in every work) and surfaced a second real bug: the model translating Nôm characters by their literal Chinese meaning instead of trusting Stage 1's already-correct phonetic reading — root-caused via a reading-fidelity proxy across the full ~115-line "unexplained" bucket, plus a smaller proper-noun/fabrication pattern (names like "Hồ công" getting genericized to "Ông"). Checked and confirmed this second bug does NOT extend to DVSKTT (its own "unexplained" bucket is 77.4% terse chronicle-header fragments already translated correctly, just hitting the known leaf-level granularity limitation). Both fixes added to `SYSTEM_PROMPT` and validated together against the real API (156-line sample, $0.42).

   **Full corpus regenerated with both fixes**: $22.22 (higher than the ~$14 estimate — longer prompt, more input tokens per call; the run was also interrupted partway by an account-level API usage cap and resumed cleanly via `--resume` once raised), 0 errors. Full-scale, not sampled: low-confidence rate 36.7%→24.7%; Kiều edit_similarity 0.609→0.768, word_jaccard 0.469→0.607; Lục Vân Tiên edit_similarity 0.584→0.741, word_jaccard 0.403→0.523; DVSKTT word_recall flat at 0.699→0.709 as expected (the fixes are poetry-only by design). Pre-fix corpus preserved as `data/translations_full_old_prompt.json`. See `experiments/phase3_translation/results.md`.
5. **Wrap the pipeline in an API, then a frontend** (Phase 4/5).
   **Phase 4 done.** Detection was a real gap first (Phases 0-3 only ever ran on already-cropped line images) — validated in rungs before writing API code: kraken's generic segmenter + a merge heuristic (Rung 0) didn't generalize across scan sources (0.43/0.48 recall/precision, 4/15 sampled pages at 0.000/0.000); fine-tuning kraken's segmenter on a 1,191-page, all-9-works stratified sample on Kaggle GPU (Rung 1) passed — mean recall 0.902, precision 0.887, the 4 previously-failing pages now 0.917-1.000. See `experiments/phase4_detection/README.md`. `api/` (FastAPI) then wraps detection→recognition(+Phase 2b correction)→opt-in translation behind `POST /v1/ocr`, reusing every existing library in place, 18 passing tests.
   **Phase 5 in progress.** A mobile *web* app (PWA) — camera capture via `<input type="file" capture="environment">` (opens the native camera app; confirmed this needs no HTTPS unlike `getUserMedia`, and avoids a real WebKit bug breaking `getUserMedia` in installed-PWA mode on iOS), served by the same FastAPI process (no CORS). Verified against the real pipeline end-to-end (not mocked) via the browser tool; not yet run on a real phone or with a real Anthropic-backed translate call. See `web/README.md`.

## Conventions for this repo

- Python for all model/pipeline code (matches upstream projects' language, makes reusing their code straightforward).
- Keep experiment scripts (Phase 0 validation, evaluation runs) in an `experiments/` folder, separate from any production pipeline code — this is research-stage work and the two shouldn't be conflated.
- Every model change should be evaluated against the same held-out test set for fair comparison — don't report accuracy numbers that used different test data between runs.
- Be explicit in commit messages and code comments about which phase (1-4 above) a piece of work belongs to.
- Flag clearly in code/PRs when something is untested assumption vs. verified result — this project's core premise is a hypothesis, not a given.

## What NOT to do

- Don't assume Chữ Nôm-specific characters will be recognized by CHAT alone — only the Chữ Hán (pure Chinese-derived) subset is expected to benefit from it.
- Don't report a model as "working" based on the training/sample data alone — always check held-out test performance.
- Don't assume a browser API works the same locally as it will on a real device — e.g. `getUserMedia` needs a secure context and has a real iOS-standalone-mode bug; verify against actual documentation/behavior before designing around it, as done for Phase 5.
